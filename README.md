...
- **Production-readiness security & audit work** — A systematic, prioritized (P0 → P1 → P2/P3) hardening pass coordinated through `PRODUCTION_READINESS_AUDIT.md` (with `PRODUCTION_READINESS.md`, `PRODUCTION_SCORECARD.md`, `SECURITY_MODEL.md`, `API_ROUTE_INTEGRITY.md`, `ROUTE_INTEGRITY.md`, and `DATABASE_MIGRATIONS.md` as supporting artifacts). Highlights:
  ...
  - **Versioned migrations** — New single-schema-boot versioned migration runner replaces ad-hoc DDL; migrations `0001`–`0009` (WhatsApp tables, money `NUMERIC`, warranty claims, assets/service history, indexes, `stock_levels` uniqueness, repair–warranty links, FK integrity, warranty coverage) run automatically on boot but **must be applied over a backed-up DB** on existing deployments. See `DATABASE_MIGRATIONS.md` and `PRODUCTION_READINESS_AUDIT.md` for details and remaining follow-ups (httpOnly cookies, per-session revocation UI, step-up auth, scheduled backups/off-site restore runbook, an automated test suite, observability, and a11y/perf items).
