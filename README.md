...
| **Colors** | `--text`, `--text-secondary`, `--text-tertiary`, `--bg`, `--bg-secondary`, `--surface`, `--border`, `--primary` (Till Orange: `#c2410c` light / `#f97316` dark), `--primary-subtle`, semantic colors (`--success`, `--danger`, `--warning`, `--info`) with light variants and `-text` contrast variants (`--success-text`, `--warning-text`, `--danger-text`), plus warm-stone `--muted` and brand `--violet` |
