...
| **Colors** | `--text`, `--text-secondary`, `--text-tertiary`, `--bg`, `--bg-secondary`, `--surface`, `--border`, `--primary`, `--primary-subtle`, semantic colors (`--success`, `--danger`, `--warning`, `--info`) with light variants and `-text` contrast variants (`--success-text`, `--warning-text`, `--danger-text`), plus neutral `--muted` and brand `--violet` |
