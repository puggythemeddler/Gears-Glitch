...
- Admin-controllable hero section with badge, headline, CTA buttons, category chips, live stats from your data, WhatsApp chat CTA, payment & delivery trust strip, on/off toggle, and a calm flat design (glow, particles, glassmorphism, and countdown effects removed) with theme-aware background colors (follows the visitor's device dark/light theme automatically, with per-theme custom color pickers)
- Catalog listings with sort controls (newest, price, name), filters, and 12-per-page pagination on category, group, campaign, and home pages
...
- Purchase order management per supplier with inline per-item receiving that actually adds stock on hand, serial intake (scan or auto-generate) for serial-tracked lines, one-click recall to fully reverse a receipt, branded PDF generation, soft-delete with completed/deleted views, and one-click restore
- Supplier directory with linked purchase orders and stock replenishment tracking
- Receiving a PO updates `stock_levels` with `purchase_receive` movement records; recalling or deleting a received PO reverses stock with `purchase_reverse` records
- Serial number tracking — per-product serial tracking with category-prefix generation, POS barcode scanning and linking at sale, automatic warranty-expiry calculation, and a Serial Numbers admin panel with search, lookup, batch generation, and void
...
- Audit log of admin/owner, staff, customer, and auth actions — filterable (user, action, entity type, date range) with pagination and detail view; accessible from the admin Activity group
...
- Customer lifecycle notifications — personalized email + WhatsApp messages on welcome (`customer.created`), order processed (with each device's serial and warranty dates), repair and warranty-claim status updates, and configurable warranty-expiry reminders (default 30 days, swept on boot and every 6 hours). Every device in an order is consolidated into one message; the greeting uses the customer's name; unknown device/warranty details are omitted rather than invented
- Per-customer notification opt-outs (email / WhatsApp) and a customer-facing notification timeline on the dashboard, alongside the admin notification log; deterministic idempotency keys plus a durable per-channel sent-log mean an already-sent channel is skipped and only a failed channel is retried
...
- **Customer lifecycle notifications** — The notification system now speaks to customers, not just staff. New business events (`customer.created` welcome, `order.processed`, `repair.customer_update`, `warranty.customer_update`, `warranty.expiry_reminder`, optional `warranty.expired`) drive personalized email + WhatsApp messages built from authoritative order/order-item/serial/warranty records. Each order is consolidated into a single message listing every device with its serial and warranty status/dates (unknown details are omitted, never invented), and greetings use the customer's name. Customers manage their own email/WhatsApp opt-outs (`customers.comm_prefs`) and see their full notification timeline on the dashboard; the admin Settings → Notifications panel gained the reminder-days control and a Send Customer Test button. Delivery is hardened with deterministic idempotency keys and a durable per-channel sent-log (`notification_log.customer_id`/`idempotency_key`/`sent_at`), so an already-sent channel is skipped while only a failed channel is retried. See `NOTIFICATION_SYSTEM.md`.
- **Comprehensive audit logging & provisioning guard** — A deploy-readiness pass that closed two silent-failure gaps:
  - **Expanded audit coverage** — A global audit middleware (`defineAudit` + an `auditSpecs` registry wired into `res.on("finish")`, logging only 2xx responses) now records **staff/admin** mutations across products, categories, groups, subcategories, coupons, gift cards, campaigns, suppliers, customers, repairs, purchases, stock + transfers, serials, stock take, spec templates, roles, staff, providers, credit notes, settings, layouts, About Us, splashes, and WhatsApp templates, plus **customer-initiated** events (orders, pending orders, quotes, loyalty redemptions, reviews, messages — attributed to `actor_role='customer'`) and **auth events** (staff/admin/password & Google login, provider login, logout, customer register/login via password/Google/magic, 2FA enable/disable, step-up usage, and password resets). Entries carry `actor_role` so non-owner views can exclude admin actions. **Audit Log page rebuilt** with filters (user, action, entity type, from/to dates) and pagination backed by the rewritten `getAuditLog()` (returns `{entries, total}`).
  - **Control-plane provisioning guard** — `POST /api/clients` now rejects provisioning with a `400 {error, missing}` before inserting a placeholder row when any of the three required keys (`NEON_API_KEY`, `RENDER_API_KEY`, `VERCEL_TOKEN`) is absent, instead of silently letting a client land in `status='failed'`. A new `GET /api/provisioning/status` endpoint + a banner in the Add Client modal surface the missing keys, a startup log line confirms/denies provisioning readiness.
  - **Report trend chart fixes** — The Daily Revenue Trend and Daily Visits Trend SVG charts gained y-axis ticks/gridlines, compact K/M value labels, date (DD/MM) labels sampled across the whole range (previously hidden beyond 14 days and cramped at 12px bars for long ranges), and captions stating the metric, period, and how to interpret the bars.
  - **Deploy & smoke-test checklist** — `DEPLOY_CHECKLIST.md` documents the three-service deploy order (backend + control plane on Render, frontend on Vercel) with exact build/start commands, the JWT_SECRET rotation, the provisioning keys, and the post-deploy smoke tests.
- **Production-readiness security & audit work** — A systematic, prioritized (P0 → P1 → P2/P3) hardening pass coordinated through `PRODUCTION_READINESS_AUDIT.md` (with `PRODUCTION_READINESS.md`, `PRODUCTION_SCORECARD.md`, `SECURITY_MODEL.md`, `API_ROUTE_INTEGRITY.md`, `ROUTE_INTEGRITY.md`, and `DATABASE_MIGRATIONS.md` as supporting artifacts). Highlights:
  - **Session rotation on password change (A-1)** — `verifySessionToken` now rejects any session token whose `iat` predates the account's `password_changed_at`; all six auth middlewares are async and enforce this, so every password change/reset invalidates existing sessions.
  - **Single-use auth tokens (A-3)** — All magic-login / admin-password-reset / password-reset flows sign short-lived (2h) tokens with a `jti` and consume them single-use via `consumeAuthToken`, so a captured token can't be replayed.
  - **Open-link token leakage (S-5)** — Receipt / purchase-PDF / quote-PDF routes now authenticate through `requirePdfAuth`: they accept a short-lived purpose token (minted via `POST /api/admin/pos-receipt-token|purchase-pdf-token|quote-pdf-token`) **or** a full session token from the `Authorization` header only — query-string session tokens are rejected. Frontend links now mint + `window.open` instead of carrying tokens in the URL.
  - **Login lockout (A-2)** — All three login functions enforce per-account failure lockout (and clear on success after 2FA).
  - **Ownership-scoped messaging (S-8)** — `PATCH /api/messages/:id/read` is scoped to the message's customer/provider.
  - **Credential redaction (S-6/P-4)** — `GET /api/mpesa/config` redacts `consumerSecret`/`passkey`; the save handler keeps existing credentials on blank submit. **Nosniff (S-10)** on `/uploads`; SVG magic-byte signatures removed from upload validation.
  - **Staff reset gating (S-9)** — `POST /api/staff/:id/reset-password` now requires `staff:update`; `/api/plans/all` filters inactive plans.
  - **Repair feature gating (Z-4)** — `requireShopFeature("Repair ticketing")` guards all 17 repair routes (Warranty, Assets, Service History sub-routes included).
  - **httpOnly session cookie (A-1)** — The session JWT now lives in an httpOnly, SameSite=Lax, secure-in-production `gg_session` cookie set on all six auth routes. `getBearerToken` falls back to it after the `Authorization` header; new `GET /api/auth/session` + `POST /api/auth/logout` endpoints let the SPA bootstrap/clear state from the cookie. The frontend no longer persists JWTs to `localStorage` for new logins (role/permission markers only, `credentials: "include"`, legacy header/tab-token paths kept for rollout).
  - **RBAC on stateful routes (Z-1)** — `requirePermission(...)` guards 143 admin/staff routes, backed by `hasPermission` with a base-role fallback so staff never lock themselves out of their own role's actions.
  - **Versioned migrations** — New single-schema-boot versioned migration runner replaces ad-hoc DDL; migrations `0001`–`0009` (WhatsApp tables, money `NUMERIC`, warranty claims, assets/service history, indexes, `stock_levels` uniqueness, repair–warranty links, FK integrity, warranty coverage) run automatically on boot but **must be applied over a backed-up DB** on existing deployments. See `DATABASE_MIGRATIONS.md`, `PRODUCTION_READINESS_AUDIT.md` (54 resolved, 7 deliberate/partial, 1 owner action — the Render free tier / C-8) and `TEST_PLAN.md` for details. Remaining optional follow-ups: per-session revocation UI, a dedicated `jti` on session JWTs, scheduled backups / off-site restore runbook, an automated test suite, observability, and a11y/perf items.
- **14-phase UX audit implemented** — A complete professional-business look-and-feel pass across storefront, admin, POS, inventory, and repairs. **Warranty admin panel**: new **Warranty** view in the staff portal showing every serialized sale with active / expiring-within-30-days / expired buckets, search, and an inline coverage record (see Key Panels). **Repairs overhaul**: tickets now show a **visual status timeline** (Received → Diagnosed → Waiting for parts → Repair in progress → Quality check → Ready → Collected), a quality-check stage, a customer **asset card** (device type, color, accessories, issue, serial/lot numbers), and **serial/warranty linkage** right on the ticket. **Customer 360**: the Customers detail page links repair history, warranty coverage, quotations, orders, and messages in one profile. **Storefront hero de-glass**: glow/particles/glassmorphism and the sale countdown were removed — the hero is a calm flat design that follows the visitor's device theme (see Sales & storefront). **Catalog listings** gained sort controls (newest, price, name), filters, and 12-per-page pagination. **Admin polish**: static (non-animated) stat cards, per-page titles throughout, a unified `formatPrice` helper, off-spec `--violet`/`--indigo` tokens removed in favor of semantic status colors plus a reusable **`--focus-ring`** outline, consistent save/settings terminology, remaining inline hex converted to tokens, and zero emoji left anywhere (every emoji replaced with SVG icons). Completed in 14 phased commits and recorded in `UX_IMPLEMENTATION_REPORT.md`.
- **Customer management in the admin panel** — Customer profiles are now fully manageable from the **Customers** panel. **Edit** opens an inline panel to update name, email, phone, or reset the password (leave blank to keep the current one); duplicate emails are rejected with a clear error. **Delete** now works for every customer regardless of history — the deletion runs in a single transaction that first cleans up all dependent records (repair tickets with their parts/updates/images, messages, product reviews, loyalty points and transactions, coupon usage, credit notes, eTIMS sales records, orders with items/invoices/gift-card redemptions/refunds, carts, wishlist, quotes, and cart-recovery reminders) before removing the customer, so no orphaned rows and no foreign-key errors.
- **POS till hardening & paged product grid** — The point of sale now charges the exact sale price it displays (effective pricing is applied server-side for line totals and order items, so receipts and the reported subtotal can't drift from the sticker). M-Pesa sales no longer fake-complete: checkout returns the STK push state and the till holds the cart in a locked "awaiting payment" panel with **verify / retry prompt / switch to cash** actions until payment is confirmed — plus a confirmation dialog (total, method, change due) before any charge, and a per-cart idempotency key reused across M-Pesa retries so a sale can never be double-charged. New **80mm thermal receipt** printing joins the A4 invoice and PDF download. The marketing shell no longer wraps the till (POS is a full-viewport, header-free layout), and large categories are **paginated 12 products per page** with Prev/Next controls so product prices never fall out of the visible area. Receipt print tabs authenticate via an opt-in query token.
- **Serial number tracking, end to end** — Products can be flagged for serial tracking (`serial_tracking`). Serial numbers are generated from category-prefixed sequences (`serial_sequences`) or scanned/typed at POS checkout, linked to the order, and drive warranty expiry dates. A **Serial Numbers** admin panel lists every serial (product, status, warranty), looks up any serial by scanning, generates batches, and voids mistakes. Fixed in this round: the serial list/lookup queries referenced a non-existent `orders.order_number` column and returned an internal server error — they now fall back to the order's invoice number (or order ID).
- **Purchase orders receive into stock** — "Mark All Received" is no longer a status-only change: it receives every line (updating received quantities, adding stock on hand with `purchase_receive` movement records, and auto-generating serials for serial-tracked lines that don't have any). New **Recall** button on any PO with received quantities fully reverses a receipt — deducts stock, voids the PO's in-stock serials, resets received quantities, and returns it to `ordered`. Deleting a PO now also reverses its stock before moving it to the Deleted tab. Per-line receiving supports barcode-scanner serial entry with a count-guard modal plus bulk serial generation.
- **Logo position honored on every PDF** — `renderStoreLogo` now renders a full-width wrapper so top-left/top-middle/top-right placement actually works on invoices, receipts, credit notes, quotes, and purchase order PDFs. PO/GRV PDFs title themselves **PURCHASE ORDER** or **GOODS RECEIVED VOUCHER** (based on status), show a store-name header with a status badge, use the store's currency for totals, list serials under received products, and download with matching `PO-…`/`GRV-…` filenames.
- **Test-site-first deploys** — Every push to `main` now auto-deploys to a single designated **test site** only; rolling out to all production clients is a deliberate manual action. The control plane gained an `is_test` flag (one client at a time, `Set as Test Site` in the client row, **TEST SITE** badge), a `POST /api/deploy-test` endpoint (deploys only the test site via Render), and a Deploy Log that records each deployment's **git commit ID and message** with a source tag (`GitHub push` vs `Manual`). The Deploy Log tab is filterable by status and searchable by client/commit. GitHub Actions now has two workflows: `.github/workflows/deploy-test.yml` (automatic on push → test site) and `.github/workflows/deploy-all-clients.yml` (manual-only, requires typing `DEPLOY` to confirm, no push trigger), both routing through the control plane so every deploy is logged with its commit.
- **Design-token rebrand to Till Orange on warm-black** — The default color palette was rebranded from slate/blue (Tailwind default look) to a distinctive warm-stone + Till Orange identity ("The Workshop Bench"). Light theme: warm off-white canvas (#fafaf9), deep orange primary (#c2410c, white text at 5.2:1 contrast). Dark theme: warm near-black canvas (#0b0a09), bright orange primary (#f97316, black text at ~8:1 contrast). Both themes use warm stone neutrals, not generic gray. Warning shifted amber→yellow to avoid collision with the orange primary. WCAG AA verified across all token pairs. New **`--muted`** token (warm stone: `#716d68` light / `#87817a` dark), both themes. `DESIGN.md` and `.impeccable/design.json` sidecar capture the full visual system. Dead `var(--x, #hex)` fallbacks cleaned up; data-driven colors (flag palettes, theme swatches, banner presets, SVG fills, `_document.tsx` theme-color) intentionally kept as literal hex.
- **Marketing page professional redesign** — The `/marketing` landing page was redesigned from a glow/particle/letter-reveal template look to a professional B2B editorial layout: sticky header with brand mark + nav + CTA, Archivo display font (deliberately not Sora, to differentiate from comparable sites), a drawn SVG icon system replacing all emoji, a feature category filter (All / Sell / Stock / Service / Customers / Platform), a stats band in the brand warm-black with orange numerals, semantic markup (figure/blockquote/nav), and a 3-column footer. Content preserved; presentation rebuilt. Requires `NEXT_PUBLIC_MARKETING_ENABLED=true` on the main Vercel deployment.
- **Admin sidebar overhaul & operational dashboard** — The staff portal sidebar was rebuilt as a full-viewport rail (232px, sticky, 100vh) with a filter-search box, icons per item (shared SVG icon system), tinted active state (primary-subtle + Till Orange text, `aria-current`), collapsible groups with SVG chevrons, and sign-out pinned to the bottom. The admin dashboard gained an operational layer: today's revenue/orders, a needs-attention panel (low stock, repairs ready, unread messages, pending subscription requests), a low-stock top-5 list, and a repair pipeline (counts per status). All cards are permission-aware and clickable into their target pages.
- **Impeccable design passes** — A series of impeccable design-system passes were run: `colorize` (rebrand + WCAG contrast audit), `audit` (POS scored 16/20 → 30/40 after fixes), `enhance` (animate/typeset/delight/layout — sale-complete pulse, tabular numerals, panel stagger), `polish` (critique findings resolved), and `document` (`DESIGN.md` + `.impeccable/design.json` sidecar capturing "The Workshop Bench" visual world). The critique snapshot is persisted in `.impeccable/critique/`.
...
- **Selectable storefront themes** — Three switchable color schemes from a **Store Theme** panel in the staff portal: **Default** (Till Orange on warm-black), **Kenyan** (green primary with red & black accents, inspired by the Kenyan flag), and **Modern** (violet + cyan). Clicking a theme previews it instantly (applied as a `data-brand-theme` attribute) and saves to `store_theme` in `/api/admin/storefront-layout` for every visitor. All hero, badge, and repair-card colors are driven by theme CSS variables, including dark-mode variants, and a reusable **`--focus-ring`** outline token (`0 0 0 3px var(--primary-subtle)`) powers every keyboard focus indicator. The hero section itself now follows the visitor's device theme automatically (a light hero with dark text in light mode, the dark hero in dark mode) and can be overridden per theme with custom background colors from the hero form. The dual "Shop premium tech / Need a repair?" identity band under the hero is now admin-toggleable ("Show shop & repair band") via the hero form.
...
- **Client-created plan auto-sync** — Client admins can create/edit plans on their own backend, and every plan has a **"Sync to other clients"** checkbox (default on). When a synced plan is saved, the client pushes it to the control plane (`POST /api/plans/sync-up`, authenticated with its own `CONTROL_PLANE_SECRET` via `x-control-plane-key`), which upserts it into `custom_plans` and distributes it to every other active client (the origin is skipped). Plans without the checkbox stay local to the creating client. The control-plane plan editor has the same checkbox plus a "Syncs to clients" badge, and clients now receive a `CONTROL_PLANE_URL` env var (provisioning + **Push Secret**) so they know where to call back. Feature catalogs (both CP and client plan editors) were also completed with the four previously-missing gated features (`Gift cards`, `Campaign pages`, `Cart recovery`, `Visitor analytics`), and the control-plane plan editor is now a full-screen modal.
...
- **Hero section admin control** — Homepage hero section (badge text, headline, subtitle, CTA buttons, category chips, stats, highlights, trust text) is now fully configurable from the admin Storefront panel. Toggle the announcement badge on/off, edit all text, add/remove category chips and stats, and set custom background colors for light and dark themes (empty = automatically follow each visitor's device theme). Config stored in `hero_config` and served to all storefront layouts via the layout context.
...
- **CSRF token protection** — Double-submit cookie pattern on all state-changing endpoints (POST/PUT/DELETE/PATCH). Frontend fetches a CSRF token on app mount via `GET /api/csrf-token` (and lazily on demand) and sends it as `X-CSRF-Token` header on every mutating request. Server **hard-fails** mutating requests with `403` when the token is missing or mismatched (external webhooks — M-Pesa callback, WhatsApp — and POS/control-plane requests are exempt).
- **Control plane CSP** — The control plane serves with a Content Security Policy (scripts/styles inline only, images from `https:` for QR codes, `frame-ancestors 'none'` to block clickjacking, `object-src 'none'`). `scriptSrcAttr: ["'self'", "'unsafe-inline'"]` is set explicitly: Helmet 7 otherwise appends `script-src-attr 'none'`, which blocks the dashboard's inline `onclick` buttons.
...
- **Control plane per-user authentication** — Control plane now has proper user accounts with login screen. Default admin user auto-created on first boot (`admin` / `gearglitch2024` or `CP_ADMIN_PASSWORD` env var). JWT-based session auth (7-day expiry) plus per-user API keys for programmatic access. Users tab for admin to manage accounts (create/delete, assign roles, regenerate API keys). Roles: `admin` (full access) and `viewer` (read-only). The legacy `CONTROL_PLANE_API_KEY` env value is no longer accepted — programmatic calls must use a per-user API key (`x-api-key`, admin role needed for deploy routes).
...
- **Premium hero section** — Original storefront layout now features a full-width two-column hero with animated orange/amber glows, floating particles, glassmorphism buttons and stat cards, auto-rotating featured product carousel (5s interval with dot navigation), floating category chips (auto-synced with the category list), "Trusted by 5,000+ customers" trust bar with gold stars, and SVG wave transition into the product grid. The background and text palette adapt to the visitor's dark/light theme automatically, or can be set to custom colors per theme from the admin hero form. Fully responsive (stacks vertically on mobile). Respects `prefers-reduced-motion`.
...
- **Repairs** — Full repair service management under Services: **Tickets** (list with status/technician filters; open a ticket to edit status, assigned technician, ETA, calendar schedule, diagnosis, costs, parts used, and add customer-visible notes; send quotes), **Calendar** (week-at-a-glance of scheduled repairs, click to jump to the ticket), and **Page Content** (edit the intro and service panels shown on the public `/repairs` page; toggle each panel visible/hidden; turn any panel into a **booking button** that opens `/repair-book` pre-filled with device type, issue description, symptoms, and a repair type for automatic pricing). Ticket details now show a **visual status timeline** (Received → Diagnosed → Waiting for parts → Repair in progress → Quality check → Ready → Collected), a status strip colorized with semantic status tokens, a **quality-check stage** ("Into Quality Check" / "Out of Quality Check") with timestamps, a **customer asset card** (device type, color, accessories, issue description, serial/lot numbers), and **serial/warranty linkage** (thumbnail, product, warranty status + view-warranty link). Feature-gated ("Repair ticketing").
- **Warranty** — Dedicated **Warranty** admin panel: a sortable register of every serialized sale (product, serial number, customer, order, purchase date, coverage in months, expiry), grouped into **Active / Expiring within 30 days / Expired** status buckets with a status filter and search across product, customer, and serial. An inline coverage panel shows the full record (customer, serial, order, coverage period, remaining days) for any row. The component is reusable — it's embedded per-customer on the **Customers** detail page so a customer's whole warranty footprint shows on their profile. Repair tickets surface the linked serial's warranty right on the ticket. Feature-gated ("Repair ticketing").
...
- **Purchases** — Create and manage purchase orders with supplier selection, product line items, status workflow (pending → ordered → received), inline per-item receiving with quantity inputs that adds stock on hand, per-line serial intake (scan/type with a count-guard modal or auto-generate batches), one-click **Mark All Received** (adds all remaining quantities to stock, auto-generating serials where needed), a **Recall** button that fully reverses a received PO, branded PDF generation/download, soft-delete with completed/deleted views (deleting a received PO reverses its stock), and one-click restore. Serial-tracked units are generated, linked to the PO, and queryable from the Serial Numbers panel
...
Documentation lives in `README.md` (this file), `LOCAL_SETUP.md`, `DEPLOY_CHECKLIST.md`, `GITHUB_ACTION_POINTS.md`, `ACTION_ITEMS.md`, `eTIMS_INTEGRATION.md`, `ROADMAP.md`, `user manual/PROJECT_MANUAL.md`, `control-plane/README.md`, and `TEST_PLAN.md` (manual regression checklist — see **D2** for the full "provision a new client to resell hardware" end-to-end test).
...
| POST | `/api/purchases/:id/recall` | Reverse a receipt — deducts stock, voids the PO's in-stock serials, resets received quantities, returns the PO to `ordered` |
| POST | `/api/purchases/:id/receive-all` | Receive every remaining line (adds stock on hand, auto-generates serials where needed) and mark the PO received |
| POST | `/api/purchases/items/:itemId/receive` | Receive a quantity for a single PO line (optionally with serial numbers) |
| GET | `/api/purchases/:id/pdf` | Download branded PDF of a purchase order (accepts query token) |
| GET | `/api/serials` | List serials (filter by `?q=`, `?status=`, `?productId=`) |
| GET | `/api/serials/lookup/:serialNumber` | Look up a serial by number (product, status, warranty, sale info) |
| POST | `/api/serials` | Add a serial manually |
| POST | `/api/serials/generate` | Generate a batch of serials for a product (optional `purchaseOrderItemId` links them to a PO line) |
| POST | `/api/serials/:id/void` | Void an in-stock serial |
| POST | `/api/serials/link-by-number` | Link a serial to an order item (POS checkout) |
...
| `MPESA_CALLBACK_SECRET` | | Shared secret; when set, `/api/mpesa/callback` requires it in the `X-Callback-Secret` header (strongly recommended in production) |
...
- **Product Deletion**: Product deletion is guarded to protect financial history. `order_items` and `stock_levels` reference products with `ON DELETE RESTRICT`, so a product referenced by orders or stock cannot be deleted — the API returns 409 and directs admins to **deactivate** instead (deactivation keeps order, quote, and stock history intact).
...
- **Coupons**: Admin can create percentage or fixed discount codes with min order, max uses, and expiry. Customers apply at checkout; each successful redemption is recorded per order in `coupon_usage` (customer + discount amount), and `used_count` is incremented atomically so `max_uses` is race-safe.
