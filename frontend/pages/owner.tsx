...
function OwnerQuotes() {
    ...
    return (
      <>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h2>Quote {created.quoteNumber}</h2>
          <button className="btn btn-sm btn-ghost" onClick={() => { setCreated(null); setCustomerId(null); setNotes(""); setItems([]); }}>New quote</button>
        </div>
        <div className="panel">
          <p><strong>Customer:</strong> {escapeHtml(customers.find((c) => c.id === created.customerId)?.name || "—")}</p>
          <p><strong>Status:</strong> {created.status}</p>
          <p><strong>Total:</strong> {formatPrice(created.total)}</p>
          {created.notes && <p><strong>Notes:</strong> {escapeHtml(created.notes)}</p>}
  ...
  return (
    <>
      <h2>Create quote</h2>
      <p className="muted">Create a hardware quote for a customer.</p>
      <div className="panel" style={{ maxWidth: 700 }}>
        ...
        {items.length === 0 ? (
          <p className="muted">No items added yet. Search and add products above.</p>
        ) : (
          <div className="table-wrap">
            <table className="data-table">
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td><input type="number" min={0} value={item.unitPrice} onChange={(e) => updateItem(idx, "unitPrice", e.target.value)} style={{ width: 100 }} /></td>
                    <td>{formatPrice(item.quantity * item.unitPrice)}</td>
                    <td><button className="btn btn-sm btn-ghost" onClick={() => removeItem(idx)}>✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem" }}>
          <RippleButton onClick={createQuote} disabled={!customerId || items.length === 0} loading={creating}>
            Create quote
          </RippleButton>
        </div>
      </div>
