...
const SOLUTIONS_BENEFITS = [
  ...
  "3 storefront layouts plus a JSON layout builder",
...
const FEATURES: { icon: string; name: string; desc: string; cat: FeatureCat }[] = [
  ...
  { icon: "users", cat: "customers", name: "Customer Management", desc: "Customer profiles with purchase history, repair records, order history, and communication log. Edit contact details or reset passwords in place, and delete any customer with full data cleanup — no orphaned records, no foreign-key errors." },
  ...
  { icon: "store", cat: "customers", name: "Online Storefront", desc: "3 storefront layouts (Original, Amazon-style, Jumia-style) plus a runtime JSON layout builder for admin-created custom themes. Admin-controllable hero sections (on/off toggle, theme-aware background that follows each visitor's device dark/light theme, and per-theme custom color pickers), live stats from your data, auto-synced category chips, product catalog, auto-rotating carousels, springboard category menu, and an About Us page with owner-editable content." },
